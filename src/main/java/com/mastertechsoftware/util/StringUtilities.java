...
public class StringUtilities {
	...
	public static String md5NonHex(String s) {
		try {
			byte[] bytes = digest.digest(s.getBytes("UTF-8"));
			StringBuilder b = new StringBuilder(32);
			for (byte aByte : bytes) {
				String byteString = Integer.toString((int) aByte);
				b.append(byteString);
			}
			return b.toString();
		} catch (UnsupportedEncodingException e) {
	 ...
	 * Note that this is no guarantee that the returned name will be definitely valid.
	 *
