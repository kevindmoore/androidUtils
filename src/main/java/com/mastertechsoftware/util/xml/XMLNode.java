...
//            XMLNode node = (XMLNode) childNodes.get(i);
       ...
       for (String key : keys) {
           ...
           for (XMLNode node : xmlNodes) {
                if (!(childName.equals(node.getNodeName()))) {
                    XMLNode childrenWithAttribute = node.getChildNodeAttribute(childName, attribute);
                    if (childrenWithAttribute != null) {
                ...
                if (attributes != null) {
                    ...
                    while (attribIter.hasNext()) {
                        String attributeName = attribIter.next();
    ...
    /**
     * Find all the children with the given child name, attribute and attribute value
     * @param childName
     * @param attribute
     * @param attributeValue
     * @return List<XMLNode>
     */
    public List<XMLNode> findChildrenWithAttribute(String childName, String attribute, String attributeValue) {
        ...
        for (String key : keys) {
            ...
            for (XMLNode node : xmlNodes) {
                if (!(childName.equals(node.getNodeName()))) {
                    List<XMLNode> childrenWithAttribute = node.findChildrenWithAttribute(childName, attribute, attributeValue);
                    if (childrenWithAttribute != null && childrenWithAttribute.size() > 0) {
                        childNodeList.addAll(childrenWithAttribute);
                    }
                    continue;
                }
                Map<String, String> attributes = node.getAttributes();
                if (attributes != null) {
                    ...
                    while (attribIter.hasNext()) {
                        ...
                        if (attribute.equals(attributeName) && attributeValue.equalsIgnoreCase(value)) {
