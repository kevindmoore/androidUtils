...
public class Id3v24TagReader {
	...
	private Id3Frame createId3Frame(String field, byte[] data, byte version)
		...
		else if (field.startsWith("PIC"))	// For id3v2.2 which has a different image format
			return new ApicId3Frame(data, version);
		// Any other frame
		else
	...
	public Id3v2Tag read(ByteBuffer data, boolean[] ID3Flags, byte version)
		...
		for (int a = 0; a < tagSize; a++) {
			...
			if (!"".equals(field)) {
				...
				if (f != null) {
					tag.add(f);
					if(f.getClass() == ApicId3Frame.class )
					{
                        try {
                            final ByteArrayInputStream bis = new ByteArrayInputStream(((
                                    ApicId3Frame)f).getData());
                            final BufferedInputStream iis = new BufferedInputStream(bis);
//                            final BufferedReader reader = new BufferedReader(iis);
//                            if (readers.hasNext()) {
//								final ImageReader reader = readers.next();
//								reader.setInput(iis);
//								final BufferedImage bim = reader.read(0);
								final int bim = iis.read();
								if (bim != -1) {
									// No exceptions, we have an image!
									tag.addCoverArt(((ApicId3Frame) f).getData());
								}
//								reader.dispose();
//							}
                            iis.close();
