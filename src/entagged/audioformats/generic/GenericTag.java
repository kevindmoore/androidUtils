...
public class GenericTag extends AbstractTag {
	...
	private class GenericTagBinaryField implements TagImageField {

		private String id;
		private byte[] content;

		GenericTagBinaryField(final String id, final byte[] initialContent) {
			this.id = id;
		...
		public void copyContent(final TagField field) {
			try {
				setContent(field.getRawContent());
		...
		public void setContent(final byte[] content) {
			this.content = content != null ? new byte[content.length] : null;
			if (content != null) {
	...
	/**
	 * Index for the &quot;cover art&quot;-identifier in {@link #keys}.
	 */
	public static final int COVER_ART = 7;
	...
	@Override
	protected TagField createCoverArtField(final byte[] content) {
		return new GenericTagBinaryField(keys[COVER_ART], content);
	}
	 ...
	 * (overridden)
	 * 
	 * @see entagged.audioformats.generic.AbstractTag#getCoverArtId()
	 */
	protected String getCoverArtId() {
		return keys[COVER_ART];
	}
